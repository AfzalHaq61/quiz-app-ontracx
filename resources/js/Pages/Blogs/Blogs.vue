<template>
  ...
  <div class="grid grid-cols-12 min-h-screen">
    ...
    <div class="col-span-10 bg-gray-100 pt-[50px] pl-[50px] pr-[50px]">
      <div>
        <BlogsHead :subject="subject"></BlogsHead>
        <div class="my-[45px]">
          ...
          <div class="grid gap-[10px]">
            <div class="grid grid-cols-8 p-[10px]">
              <div class="flex items-center">
                <div class="border-2 border-black w-[12px] h-[12px]"></div>
              </div>
              <div class="">
                <h1 class="text-[#AFAFAF]">Pic</h1>
              </div>
              <div class="col-span-2">
                <h1 class="text-[#AFAFAF]">Title</h1>
              </div>
              <div class="col-span-2">
                <h1 class="text-[#AFAFAF]">Details</h1>
              </div>
              <div>
                <h1 class="text-[#AFAFAF] ml-16">Status</h1>
              </div>
            ...
            <div
              v-for="blog in blogs"
              :key="blog.id"
              class="grid grid-cols-8 bg-white rounded-[8px] p-[10px]"
            >
              <div class="flex items-center">
                <div class="border-2 border-black w-[12px] h-[12px]"></div>
              </div>
              ...
              <div class="flex items-center ml-16">
                <div v-if="!blog.status">
                  <h1 class="text-[#1D9229]">{{ blog.status }}</h1>
                </div>
                <div v-if="blog.status">
                  <h1 class="text-[#1D9229]">{{ blog.status }}</h1>
                </div>
                  ...
                  >
                    ...
                    >
                      <div class="py-1">
                        ...
                        <MenuItem v-slot="{ active }">
                          <Link
                            :href="route('blogs.delete', { blog: blog.id })"
                            class="text-[#E31D1D]"
                            :class="[
                              active
                                ? 'bg-gray-100 text-gray-900'
                        ...
                        <div>
                          <div v-if="blog.status === 'Draft'">
                            <MenuItem v-slot="{ active }">
                              <Link
                                :href="
                                  route('blogs.publish', { blog: blog.id })
                                "
                                class="text-[#1D9229]"
                                :class="[
                                  active
                          ...
                          <div v-if="blog.status === 'Published'">
                            <MenuItem v-slot="{ active }">
                              <Link
                                :href="route('blogs.draft', { blog: blog.id })"
                                class="text-[#1D9229]"
                                :class="[
                                  active
                                    ? 'bg-gray-100 text-gray-900'
